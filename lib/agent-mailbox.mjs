...
import { PRESENCE_FILE, PRESENCE_STALE_MS, switchboardDir } from './presence.mjs';
...
const mailboxDir = (dir) => dir ?? process.env.AGENT_MAILBOX_DIR ?? MAILBOX_DIR;
const presenceFile = (file) => file ?? process.env.AGENT_MAILBOX_PRESENCE_FILE ?? PRESENCE_FILE;
...
export const mailboxKeysForSession = (sessionId, { dir } = {}) => {
  ...
  let files = [];
  try {
    files = fs.readdirSync(mailboxDir(dir)).filter((f) => f.endsWith(suffix));
  } catch {
    return [];
  }
...
export const mailboxPath = (key, { dir } = {}) => path.join(mailboxDir(dir), `${key}.jsonl`);

// Live raw presence beacons (unlike readAllPresence, keeps the `mailbox`
// consumer flag the hooks set and the sessionId the key needs).
export const readPresenceBeacons = ({ presenceFile: file, now = Date.now() } = {}) => {
  let beacons = [];
  try {
    const parsed = JSON.parse(fs.readFileSync(presenceFile(file), 'utf-8'));
...
// Every window as {id, title, tabTitle, pid, cwd}, resolved against
...
const resolveWindowRecipient = (beacons, win, sender) => {
...
export const resolveRecipient = (target, { presenceFile: file, senderSessionId = null, senderCwd = null, senderWindowId = null } = {}) => {
  ...
  if (/^\d+$/.test(String(target))) {
    ...
    return resolveWindowRecipient(beacons, win, sender);
  }
  ...
  return resolveWindowRecipient(beacons, matches[0], sender);
};
...
export const appendMessage = ({ to, from, priority, text, now = Date.now(), dir } = {}) => {
  ...
  fs.mkdirSync(mailboxDir(dir), { recursive: true });
